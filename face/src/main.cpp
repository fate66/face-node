...
//----------------------------------------------------------------------------------------
inline float
CosineDistance(const cv::Mat &v1, const cv::Mat &v2)
{
...
// painting
//----------------------------------------------------------------------------------------
void DrawObjects(cv::Mat &frame, vector<FaceObject> &Faces, Napi::Function &cb, Napi::Env &env)
{
...
//----- labels ----------------------------
        ...
        default:
            ...
            cb &&cb.Call(env.Global(), {Napi::String::New(env, Str)});
        }
...
Napi::Value CameraRecognize(const Napi::CallbackInfo &info)
{
    ...
    while (1)
    {
        ...
        DrawObjects(frame, Faces, cb, env);
...
{
    exports.Set(Napi::String::New(env, "cameraRecognize"), Napi::Function::New(env, CameraRecognize));
    return exports;
}

NODE_API_MODULE(cameraRecognize, Init)
